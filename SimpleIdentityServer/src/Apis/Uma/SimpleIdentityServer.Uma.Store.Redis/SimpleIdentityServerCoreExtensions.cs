...
namespace SimpleIdentityServer.Uma.Store.Redis
{
    ...
    {
        public static IServiceCollection AddUmaRedisStore(this IServiceCollection serviceCollection, Action<RedisCacheOptions> callback, int port = 6379)
        {
            ...
            callback(options);
            return AddUmaRedisStore(serviceCollection, options, port);
        }

        public static IServiceCollection AddUmaRedisStore(this IServiceCollection serviceCollection, RedisCacheOptions options, int port = 6379)
        {
            if (serviceCollection == null)
            {
                throw new ArgumentNullException(nameof(serviceCollection));
            ...
            serviceCollection.AddSingleton<ITicketStore>(new RedisTicketStore(redisStorage));
