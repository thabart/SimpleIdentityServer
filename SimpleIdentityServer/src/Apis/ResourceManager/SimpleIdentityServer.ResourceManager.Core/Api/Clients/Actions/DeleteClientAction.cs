...
{
    public interface IDeleteClientAction
    {
        Task<BaseResponse> Execute(string subject, string clientId, EndpointTypes type);
    }

    internal sealed class DeleteClientAction : IDeleteClientAction
    {
        ...
        public DeleteClientAction(IOpenIdManagerClientFactory openIdManagerClientFactory,
        ...
        public async Task<BaseResponse> Execute(string subject, string clientId, EndpointTypes type)
        {
            ...
            return await _openIdManagerClientFactory.GetOpenIdsClient().ResolvedDelete(new Uri(endpoint.ManagerUrl), clientId, grantedToken.AccessToken);
        }
    }
}
