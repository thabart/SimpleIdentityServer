...
# FETCH USER INFORMATION BY PASSING THE ACCESS TOKEN INTO THE HTTP BODY
Scenario: Fetch the user information for the scope profile and  the access token is passed to the HTTP body
	Given a mobile application MyHolidays is defined
	And set the name of the issuer http://localhost/identity
	And the redirection uri http://localhost is assigned to the client MyHolidays
	And the scopes are defined
...
#FETCH THE USER INFORMATION BY PASSING THE ACCESS TOKEN TO THE QUERY STRING
Scenario: Fetch the user information for the scope profile and  the access token is passed to the QUERY STRING
	Given a mobile application MyHolidays is defined
	And set the name of the issuer http://localhost/identity
	And the redirection uri http://localhost is assigned to the client MyHolidays
	And the scopes are defined
